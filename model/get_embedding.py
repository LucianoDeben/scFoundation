...
parser.add_argument(
    ...
    help="cell or gene embedding; default: cell the difference between gene and gene_batch is that in gene mode the gene embedding will be processed one by one. while in gene_batch mode, the gene embedding will be processed in batch. GEARS use gene_batch mode.",
)
parser.add_argument(
    "--pool_type",
    type=str,
...
parser.add_argument(
    ...
    type=str,
    default="t4",
    help="the targeted high resolution (start with t) or the fold change of the high resolution (start with f), or the addtion (start with a) of the high resoultion. only valid for input_type=singlecell",
)
...
def main():
    ...
    if gexpr_feature.shape[1] < 19264:
        ...
        gexpr_feature, to_fill_columns, var = main_gene_selection(
            gexpr_feature, gene_list
        )
    ...
    if args.version == "noversion":
        ...
        if args.output_type == "cell":
            ...
            raise ValueError(
                "output_mode must be one of cell gene, gene_batch, gene_expression"
            )
    ...
    for i in tqdm(range(gexpr_feature.shape[0])):
        with torch.no_grad():
            ...
            if args.input_type == "bulk":
                ...
                pretrain_gene_x = (
                    torch.tensor(tmpdata + [totalcount, totalcount]).unsqueeze(0).cuda()
                )
            ...
            # Single cell
                ...
                if args.pre_normalized == "F":
                    tmpdata = (
                        np.log1p(
                            gexpr_feature.iloc[i, :]
                            / (gexpr_feature.iloc[i, :].sum())
                            * 1e4
                        )
                    ).tolist()
                elif args.pre_normalized == "T":
                    tmpdata = (gexpr_feature.iloc[i, :]).tolist()
                    current_cell_expression = gexpr_feature.iloc[
                        i, :
                    ]  # This is log-normalized
                    totalcount_sum = current_cell_expression.sum()
                    if (
                        totalcount_sum <= 1e-9
                    ):  # Use a small epsilon for comparison if values are very small floats
                        # print(f"DEBUG: Sum of log-normalized expression for cell {i} is <=0 ({totalcount_sum}). Using 1.0 for S-token calculation base.")
                        totalcount = 1.0  # A small positive placeholder, log10(1) = 0
                    else:
                ...
                if args.tgthighres[0] == "f":
                    pretrain_gene_x = (
                        torch.tensor(
                            tmpdata
                            + [
                                np.log10(totalcount * float(args.tgthighres[1:])),
                                np.log10(totalcount),
                            ]
                        )
                        .unsqueeze(0)
                    ...
                    pretrain_gene_x = (
                        torch.tensor(
                            tmpdata + [float(args.tgthighres[1:]), np.log10(totalcount)]
                        )
                        .unsqueeze(0)
                        .cuda()
                    )
            ...
            x, x_padding = gatherData(
                pretrain_gene_x, value_labels, pretrainconfig["pad_token_id"]
            )
            ...
            # Gene embedding
                ...
                (
                    ...
                    pretrain_gene_x.float(), pretrain_gene_x.float(), pretrainconfig
                )
            ...
            # Gene_expression
                ...
                raise ValueError(
                    "output_type must be cell or gene or gene_batch or gene_expression"
                )
